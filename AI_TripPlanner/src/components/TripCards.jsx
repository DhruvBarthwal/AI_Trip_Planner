...
const TripCards = ({trip}) => {
  ...
  return (
    <div>
        <img 
            src={placePhotoUrl?placePhotoUrl:"/airplane-flying-sky-vector-illustration-paper-art-style_103044-4435.avif" }
            alt="img"
            className='rounded-[10px]'
        />
        <div>
            <h2 className='font-bold text-lg'>{trip?.userSelection?.location?.label}</h2>
            <h2 className='text-sm text-zinc-500'>{trip?.userSelection?.noOfDays} Days Trip with {trip?.userSelection?.budget} Budget</h2>
        </div>
    </div>
  )
}
